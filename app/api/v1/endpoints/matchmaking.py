...
#     # Return the FULL match result so the frontend can show the loss modal immediately
#     return {
#         "status": "forfeited",
#         "match_result": payload,
#         "xp_earned": loser.xp_earned if loser else 0,
#         "rank_points_delta": loser.rank_points_delta if loser else 0,
#     }
...
async def _try_match(
    ...
    for candidate in candidates:
        ...
        # Validate active match key – do not trust Redis blindly
        candidate_active_raw = await redis.get(_active_match_key(candidate_str))
        if candidate_active_raw:
            stale = False
            try:
                cand_match_id = candidate_active_raw.decode() if isinstance(candidate_active_raw, bytes) else candidate_active_raw
                cand_uuid = UUID(cand_match_id)
                cand_result = await db.execute(select(Match.status).where(Match.id == cand_uuid))
                cand_status = cand_result.scalar_one_or_none()
                if cand_status is None or _is_finished(cand_status):
                    # Stale key – clean it up and allow matching
                    await redis.delete(_active_match_key(candidate_str))
                    stale = True
            except Exception:
                await redis.delete(_active_match_key(candidate_str))
                stale = True
